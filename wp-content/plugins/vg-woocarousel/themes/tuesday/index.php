<?php
function vgwc_body_tuesday($post_id)
{
	...
	if($vgwc_total_items)
	{
		...
		#vgwc-wrapper'.$post_id.' .vgwc-item-i {
			margin: '.$vgwc_imargin.';			
			color:  '.$vgwc_itext_color.';'.
			(($vgwc_isibg_color) ? 'background-color: '.$vgwc_ibg_color.';' : '') .'
		}
		#vgwc-wrapper'.$post_id.' .vgwc-item-i a {
			color:  '.$vgwc_ilink_color.';
		}
	...
	/* HTML Block */
	$vgwc_html .= '<div id="vgwc-wrapper'.$post_id.'" class="vgwc-wrapper theme-tuesday owl-carousel '.$vgwc_class_sufix.'">';
		if($vgwc_total_items)
		{
