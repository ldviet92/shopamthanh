...
class Custom_Ajax_search_widget extends WP_Widget{
	function __construct(){
		...
		parent::__construct(
			...
			esc_html__('Ajax AutoSuggest', 'vg-primave'),
			array('description' => esc_html__('Ajax AutoSuggest Form', 'vg-primave'))
		);
	}
	...
	public function form($instance){ // Backend widget form, instance is user posted data
		global $codenegar_aas;
		$defaults = array(
			'title'		  => esc_html__('Search', 'vg-primave'),
			'placeholder' => esc_html__('Type Keyword...', 'vg-primave'),
			'max_width' => esc_html__('350', 'vg-primave')
		);
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('placeholder')); ?>" ><?php esc_html_e('Placeholder', 'vg-primave'); ?>:</label>
			<input class="widefat"
