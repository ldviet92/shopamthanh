<?php
function vgwc_body_primave($post_id)
{
	global $vgwc_wp_query, $road_showcountdown;
	...
	$vgwc_left_offset	= (empty($vgwc_left_offset)) ? "-15" : sanitize_text_field($vgwc_left_offset);
	...
	/* HTML Block */
	$vgwc_html .= '<div id="vgwc-wrapper'.$post_id.'" class="vgwc-wrapper vg-cateproducts theme-primave owl-carousel '.$vgwc_class_sufix.'">';
		if($vgwc_total_items)
		{
			for($i = 0; $i < $vgwc_total_loop; $i ++)
			{
				...
				for($m = 0; $m < $vgwc_row_carousel; $m ++)
				{
					...
					if(isset($primave_options['second_image']) && $primave_options['second_image']){
						...
						if(isset($attachment_ids[0]) && ($attachment_ids[0] != get_post_thumbnail_id(get_the_ID()))) {
							$vgwc_thumb_second 	 = wp_get_attachment_image_src($attachment_ids[0], $vgwc_image_size);
							$vgwc_thumb_second_url = $vgwc_thumb_second['0'];
							$image_second = '<img alt="'.get_the_title().'" data-src="'.$vgwc_thumb_second_url.'" src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw=="  class="secondary_image lazy"/>';
						}
						elseif(isset($attachment_ids[1])){
							$vgwc_thumb_second 	 = wp_get_attachment_image_src($attachment_ids[1], $vgwc_image_size);
							$vgwc_thumb_second_url = $vgwc_thumb_second['0'];
							$image_second = '<img alt="'.get_the_title().'" data-src="'.$vgwc_thumb_second_url.'" src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw=="  class="secondary_image lazy"/>';	
						}
					}
					...
					$vgwc_rating	= wc_get_rating_html( $product->get_average_rating());
					$vgwc_review	= $product->get_review_count() . esc_html__(" review(s)", "vg-primave");
					$countdown 		= false;
					$sale_end 		= get_post_meta( $product->get_id(), '_sale_price_dates_to', true );
					
					$vgwc_html .= '<div class="vgwc-item-i">';
						$vgwc_html .= '<div class="ma-box-content">';
							...
							if($vgwc_product_name || $vgwc_product_rating || $vgwc_product_price || $vgwc_add_cart)
							{
								$vgwc_html .= '<div class="vgwc-text-block">';
									...
									if($vgwc_product_rating && get_option('woocommerce_enable_review_rating') === 'yes'){
										$vgwc_html .= '<div class="vgwc-product-rating">';
										if(!$vgwc_rating)
										{
											$vgwc_html .= '<div class="star-rating"></div>';
										}else {
											$vgwc_html .= ''.$vgwc_rating.' '.$vgwc_review.'';
										}
										$vgwc_html .= '</div>';	
									}
							...
							if($vgwc_product_image)
							{
								$vgwc_html .= '<div class="vgwc-image-block">';
									$vgwc_html .= '<a href="'.get_permalink().'" title="'.get_the_title().'">';
										$vgwc_html .= '<img alt="'.get_the_title().'" data-src="'.$vgwc_thumb_url.'"  src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" class="primary_image lazy  wp-post-image"/>';		$vgwc_html .= $image_second;
									$vgwc_html .= '</a>';					
					
									$vgwc_html .= ($vgwc_product_label && $vgwc_featured) ? '<div class="vgwc-label vgwc-featured">'.esc_html__("Hot", "vg-primave").'</div>' : "";
									$vgwc_html .= ($vgwc_product_label && $vgwc_onsale) ? '<div class="vgwc-label vgwc-onsale">'.esc_html__("Sale", "vg-primave").'</div>' : "";
									
									if($vgwc_add_cart || class_exists('YITH_WCWL') || class_exists('YITH_Woocompare') || $primave_options['quick_view'])
									{
										if($primave_options['quick_view']){
											$vgwc_html .= '<div class="vgwc-quick"><a class="quick-view quickview" data-quick-id="'.$vgwc_post_id.'" href="'.get_permalink().'" title="'.get_the_title().'">';
												$vgwc_html .= esc_html__("Quick view", "vg-primave");
											$vgwc_html .= '</a></div>';
										}
										$vgwc_html .= '<div class="vgwc-button-group">';
											if($vgwc_add_cart){
												$vgwc_html .= '<div class="vgwc-add-to-cart vgwc-add-to-cart-inline">'.do_shortcode('[add_to_cart id="'.get_the_ID().'" show_price="false"]').'</div>';
											}
											$vgwc_html .= '<div class="add-to-links">';
												if(class_exists('YITH_WCWL')){
													$vgwc_html .= '<div class="vgwc-wishlist">'.preg_replace("/<img[^>]+\>/i", " ", do_shortcode('[yith_wcwl_add_to_wishlist]')).'</div>';
												}
												if(class_exists('YITH_Woocompare')) {
													$vgwc_html .= '<div class="vgwc-compare">'.do_shortcode('[yith_compare_button]').'</div>';
												}
											$vgwc_html .= '</div>';
										$vgwc_html .= '</div>';
									}
		...
		{
			return esc_html__("No item found. Please check your config!", "vg-primave");
		}
	$vgwc_html .= '</div>';
	...
	if($vgwc_total_items)
	{
		...
		jQuery(document).ready(function($) {
			$("#vgwc-wrapper'.$post_id.'").owlCarousel({
				...
				navigationText: 	["'.esc_html__("Prev", "vg-primave").'", "'.esc_html__("Next", "vg-primave").'"],
