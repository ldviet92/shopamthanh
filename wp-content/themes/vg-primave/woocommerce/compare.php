...
<head>
    ...
    <?php
		function primave_compare_css () {
			wp_enqueue_style( 'style-compare', YITH_WOOCOMPARE_ASSETS_URL . '/css/style.css' );
			wp_enqueue_style( 'jquery-colorbox', YITH_WOOCOMPARE_ASSETS_URL . '/css/colorbox.css' );
			wp_enqueue_style( 'jquery-dataTables', YITH_WOOCOMPARE_ASSETS_URL . '/css/jquery.dataTables.css' );	
		}
		
		add_action('wp_head', 'primave_compare_css');
	?>
    <?php wp_head() ?>
    ...
    <style type="text/css">
		...
		table.compare-list tr.price .old-price, table.compare-list tr.price del {
		  margin: 0;
		  color: #888888;
		  text-decoration: line-through;
		  font-size: 14px;
		}
		table.compare-list tr.price .old-price .amount, table.compare-list tr.price del .amount {
		  margin: 0 5px 0 0;
		}
		table.compare-list tr.price .special-price, table.compare-list tr.price ins {
...
<h1>
    ...
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php esc_html_e( 'Close window [X]', 'vg-primave' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php esc_html_e( 'No products added in the compare table.', 'vg-primave' ) ?></td>
        </tr>
        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                <td class="<?php echo esc_attr($product_class); ?>">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->id ) ) ?>" data-product_id="<?php echo $product->id; ?>"><?php esc_html_e( 'Remove', 'vg-primave' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
